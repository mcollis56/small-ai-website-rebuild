...
export default function ServicesPreview() {
  ...
  bg-blue-100 px-3 py-1 text-sm font-medium text-blue-800">
                      Most Popular
                    </span>
                  </div>
                )}

                <h3 className={`text-lg font-semibold ${service.popular ? 
  ...
  tracking-tight">{service.price}</span>
                  <span className={`text-sm font-semibold leading-6 
  ${service.popular ? 'text-blue-200' : 'text-gray-600'}`}>
                    one-time
                  </span>
                </p>
  ...
  ${service.popular ? 'text-blue-100' : 'text-gray-600'}`}>
                  {service.features.map((feature) => (
                    <li key={feature} className="flex gap-x-3">
                      <span className={`text-sm ${service.popular ? 
  'text-blue-200' : 'text-blue-600'}`}>✓</span>
                      {feature}
                    </li>
                  ))}
                </ul>
